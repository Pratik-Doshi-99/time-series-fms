# Autoregressive Transformer for Discretized Time Series

This repository provides a **decoder-only Transformer** for modeling **discretized time-series data**. It includes:

- **Data preprocessing** (normalization and quantization)
...
Model performance across different time-series patterns (256 timesteps, 32,512 predictions per pattern):
...
**Aggregate Statistics**: Mean Accuracy: 41.3% ± 29.1% | Mean MAE: 2.16 ± 2.91


## Data Preprocessing

All data preprocessing steps are handled by the **`TSPreprocessor`** in `data/dataset.py`. The major steps are **normalization** and **quantization**.
...
where `min_x` and `max_x` are the minimum and maximum of the entire series. If `(max_x == min_x)`, meaning the series is constant, we assign everything to zero.
...
We do the following:
1. Multiply the normalized value by `(num_classes - 1)`
2. Round to the nearest integer
3. Clip to ensure the result is within `[0..(num_classes - 1)]`
...
These pairs get batched in the **`AutoregressiveLoader`**, which also builds a **causal mask** for each prefix so the model only attends to past tokens. During training, the loss is computed only on the last token in each prefix. Both approaches use **teacher forcing** (feeding ground truth as input rather than model predictions).
...
A **causal mask** ensures that future tokens cannot be seen at each time step. We then compute the cross-entropy loss over **all** time steps in the sequence in a single forward pass. This enables **parallel training** where all predictions are made simultaneously, unlike the incremental approach which treats each prefix separately.


## Model Architecture

A standard **decoder-only Transformer** similar to GPT-style language models:

- **Embedding layer**: Maps quantized tokens to dense vectors
- **Sinusoidal positional encoding**: Standard sine/cosine encoding from "Attention Is All You Need"
- **Stack of Transformer decoder layers**: Self-attention + feedforward blocks with causal masking
- **Output projection**: Linear layer to predict next token logits

Implementation in `models/model.py`.
...
@software{time_series_fms,
  title = {Autoregressive Transformer for Discretized Time Series},
  author = {Doshi, Pratik},
  year = {2026},
  url = {https://github.com/pratikdoshi/time-series-fms},
  note = {A decoder-only Transformer for discretized time-series forecasting}
}
```